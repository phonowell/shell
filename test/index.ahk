...
global __ci_shell__ := Func("shell_195")
global $ := {}
$.abs := Func("shell_194")
$.activate := Func("shell_193")
$.add := Func("shell_192")
$.alert := Func("shell_191")
$.beep := Func("shell_189")
$.bind := Func("shell_188")
$.blockInput := Func("shell_187")
$.ceil := Func("shell_186")
$.chunk := Func("shell_185")
$.clearInterval := Func("shell_184")
$.clearTimeout := Func("shell_183")
$.click := Func("shell_182")
$.clone := Func("shell_181")
$.compact := Func("shell_180")
$.concat := Func("shell_178")
global __iDebounce__ := 0
global __tDebounce__ := {}
$.debounce := Func("shell_177")
$.defer := Func("shell_175")
$.delay := Func("shell_173")
$.drop := Func("shell_171")
$.each := Func("shell_169")
class EmitterShellX {
  bus := []
  emit := Func("shell_168").Bind(this)
  off := Func("shell_167").Bind(this)
  on := Func("shell_166").Bind(this)
}
$.emitter := Func("shell_165")
global __eq__ := Func("shell_164")
$.eq := __eq__
$.exit := Func("shell_163")
class FileShellX {
  name := ""
  ...
  append := Func("shell_162").Bind(this)
  exist := Func("shell_161").Bind(this)
  delete := Func("shell_160").Bind(this)
  load := Func("shell_159").Bind(this)
  save := Func("shell_158").Bind(this)
}
$.file := Func("shell_157")
$.filter := Func("shell_156")
$.findColor := Func("shell_155")
$.findImage := Func("shell_154")
$.first := Func("shell_153")
$.head := $.first
$.floor := Func("shell_152")
$.formatHotkey := Func("shell_151")
global __formatKeyFormatHotkey__ := Func("shell_150")
global __pickPrefixFormatHotkey__ := Func("shell_149")
$.getColor := Func("shell_148")
$.getPosition := Func("shell_147")
$.getState := Func("shell_146")
$.gt := Func("shell_145")
$.gte := Func("shell_144")
$.i := Func("shell_143")
$.includes := Func("shell_142")
$.info := Func("shell_141")
$.isArray := Func("shell_140")
$.isFunction := Func("shell_139")
$.isNumber := Func("shell_138")
$.isObject := Func("shell_137")
$.isString := Func("shell_136")
$.join := Func("shell_135")
class KeyBindingShellX {
  mapBound := {}
  mapCallback := {}
  add := Func("shell_134").Bind(this)
  fire := Func("shell_133").Bind(this)
  init := Func("shell_132").Bind(this)
  off := Func("shell_130").Bind(this)
  on := Func("shell_129").Bind(this)
  remove := Func("shell_128").Bind(this)
}
...
$.keys := Func("shell_127")
$.last := Func("shell_126")
$.length := Func("shell_125")
$.lt := Func("shell_124")
$.lte := Func("shell_123")
$.map := Func("shell_122")
$.mixin := Func("shell_121")
$.mod := Func("shell_120")
$.move := Func("shell_119")
$.noop := Func("shell_118")
$.now := Func("shell_117")
...
$.open := Func("shell_113")
$.play := Func("shell_112")
$.press := Func("shell_111")
global __formatInputPress__ := Func("shell_110")
global __validateInputPress__ := Func("shell_109")
$.push := Func("shell_108")
$.random := Func("shell_107")
$.range := Func("shell_106")
$.reload := Func("shell_105")
$.replace := Func("shell_104")
$.reverse := Func("shell_103")
$.round := Func("shell_102")
$.setFixed := Func("shell_101")
$.setInterval := Func("shell_100")
$.setTimeout := Func("shell_99")
$.shuffle := Func("shell_98").Bind(__split__)
$.sleep := Func("shell_97")
$.slice := Func("shell_96")
global __pickArgumentSlice__ := Func("shell_95")
$.sort := Func("shell_94")
$.split := Func("shell_93")
$.sum := Func("shell_92")
$.suspend := Func("shell_91")
$.tail := Func("shell_90")
$.take := Func("shell_89")
global __iThrottle__ := 0
global __tThrottle__ := {}
global __tsThrottle__ := {}
$.throttle := Func("shell_88")
$.toLowerCase := Func("shell_85")
$.toLower := $.toLowerCase
$.toString := Func("shell_84")
$.toUpperCase := Func("shell_83")
$.toUpper := $.toUpperCase
$.trim := Func("shell_82")
$.trimEnd := Func("shell_81")
$.trimStart := Func("shell_80")
$.type := Func("shell_79")
$.uniq := Func("shell_78")
$.values := Func("shell_77")
$.VERSION := "0.0.14"
(Func("shell_76")).Call()
(Func("shell_75")).Call()
(Func("shell_74")).Call()
...
shell_87(callback, time, index) {
  ...
  __tThrottle__[__ci_shell__.Call(index)] := $.setTimeout.Call(Func("shell_86").Bind(callback, index), __tsThrottle__[__ci_shell__.Call(index)] - $.now.Call() + time)
}
shell_88(time, callback) {
  __iThrottle__++
  __tsThrottle__[__ci_shell__.Call(__iThrottle__)] := 0
  return Func("shell_87").Bind(callback, time, __iThrottle__)
}
...
shell_104(input, searchment, replacement, limit := -1) {
...
shell_115(callback) {
  ...
  return Func("shell_114").Bind(__i__, callback)
}
shell_116(list, n := 0) {
  if (n >= 0) {
...
shell_119(point := "", speed := 0) {
  MouseMove, point[1], point[2], speed
}
shell_120(dividend, divisor) {
...
shell_132(this, key) {
  ...
  this.mapCallback[__ci_shell__.Call(key)] := []
  __fn__ := Func("shell_131").Bind(key, this)
...
shell_155(color, start := "", end := "", variation := 0) {
  if !(start) {
    start := [0, 0]
  }
  if !(end) {
...
shell_171(list, n := 1) {
  return $.filter.Call(list, Func("shell_170").Bind(n))
}
shell_172(callback, args) {
  callback.Call(args*)
}
shell_173(callback, time, args*) {
  __timer__ := $.setTimeout.Call(Func("shell_172").Bind(callback, args), time)
  return __timer__
}
shell_174(callback, args) {
  callback.Call(args*)
}
shell_175(callback, args*) {
  return $.delay.Call(Func("shell_174").Bind(callback, args), 1)
}
shell_176(callback, time, index) {
...
shell_177(time, callback) {
  __iDebounce__++
  return Func("shell_176").Bind(callback, time, __iDebounce__)
}
shell_178(list, args*) {
...
shell_180(list) {
  return $.filter.Call(list, Func("shell_179"))
}
shell_181(input) {
  __type__ := $.type.Call(input)
  if (__type__ == "array") {
...
shell_191(message*) {
  __msg__ := $.join.Call(($.map.Call(message, Func("shell_190"))), ", ")
  MsgBox, % __msg__
}
shell_192(args*) {
  __result__ := 0

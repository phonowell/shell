...
global __ci_shell__ := Func("shell_219")
global $ := {}
$.abs := Func("shell_218")
$.activate := Func("shell_217")
$.add := Func("shell_216")
$.alert := Func("shell_215")
$.beep := Func("shell_213")
$.bind := Func("shell_212")
$.blockInput := Func("shell_211")
$.ceil := Func("shell_210")
$.chunk := Func("shell_209")
$.clearInterval := Func("shell_208")
$.clearTimeout := Func("shell_207")
$.click := Func("shell_206")
$.clone := Func("shell_205")
$.close := Func("shell_204")
$.compact := Func("shell_203")
$.concat := Func("shell_201")
$.confirm := Func("shell_200")
global __iDebounce__ := 0
global __tDebounce__ := {}
$.debounce := Func("shell_199")
$.defer := Func("shell_197")
$.delay := Func("shell_195")
$.drop := Func("shell_193")
$.each := Func("shell_191")
class EmitterShellX {
  bus := []
  emit := Func("shell_190").Bind(this)
  off := Func("shell_189").Bind(this)
  on := Func("shell_188").Bind(this)
}
$.emitter := Func("shell_187")
global __eq__ := Func("shell_186")
$.eq := __eq__
$.exit := Func("shell_185")
class FileShellX {
  name := ""
  ...
  append := Func("shell_184").Bind(this)
  exist := Func("shell_183").Bind(this)
  delete := Func("shell_182").Bind(this)
  load := Func("shell_181").Bind(this)
  save := Func("shell_180").Bind(this)
}
$.file := Func("shell_179")
$.filter := Func("shell_178")
$.findColor := Func("shell_177")
$.findImage := Func("shell_176")
$.first := Func("shell_175")
$.head := $.first
$.floor := Func("shell_174")
$.forEach := Func("shell_173")
$.formatHotkey := Func("shell_172")
global __formatKeyFormatHotkey__ := Func("shell_171")
global __pickPrefixFormatHotkey__ := Func("shell_170")
$.get := Func("shell_169")
$.getColor := Func("shell_168")
$.getPosition := Func("shell_167")
$.getState := Func("shell_166")
$.gt := Func("shell_165")
$.gte := Func("shell_164")
$.i := Func("shell_163")
$.includes := Func("shell_162")
$.info := Func("shell_161")
$.isActive := Func("shell_160")
$.isArray := Func("shell_159")
$.isExisted := Func("shell_158")
$.isFunction := Func("shell_157")
$.isNumber := Func("shell_156")
$.isObject := Func("shell_155")
$.isString := Func("shell_154")
$.join := Func("shell_153")
class KeyBindingShellX {
  mapBound := {}
  mapCallback := {}
  add := Func("shell_152").Bind(this)
  fire := Func("shell_151").Bind(this)
...
shell_95(input, omitting := " `t") {
  return RTrim(input, omitting)
}
shell_96(input, omitting := " `t") {
...
shell_119(input, searchment, replacement, limit := -1) {
...
shell_164(value, other) {
  return value >= other
}
shell_165(value, other) {
  return value > other
}
shell_166(key) {
...
shell_177(color, start := "", end := "", variation := 0) {
  if !(start) {
    start := [0, 0]
  }
  if !(end) {
...
shell_193(list, n := 1) {
  return $.filter.Call(list, Func("shell_192").Bind(n))
}
shell_194(callback, args) {
  callback.Call(args*)
}
shell_195(callback, time, args*) {
  __timer__ := $.setTimeout.Call(Func("shell_194").Bind(callback, args), time)
  return __timer__
}
shell_196(callback, args) {
  callback.Call(args*)
}
shell_197(callback, args*) {
  return $.delay.Call(Func("shell_196").Bind(callback, args), 1)
}
shell_198(callback, time, index) {
...
shell_199(time, callback) {
  __iDebounce__++
  return Func("shell_198").Bind(callback, time, __iDebounce__)
}
shell_200(text, callback) {
...
shell_203(list) {
  return $.filter.Call(list, Func("shell_202"))
}
shell_204(target) {
  if !(target) {
...
shell_215(message*) {
  __msg__ := $.join.Call(($.map.Call(message, Func("shell_214"))), ", ")
  MsgBox, % __msg__
}
shell_216(args*) {
  __result__ := 0

...
global __ci_shell__ := Func("shell_239")
global $ := {}
$.abs := Func("shell_238")
$.activate := Func("shell_237")
$.add := Func("shell_236")
$.alert := Func("shell_235")
$.beep := Func("shell_233")
$.bind := Func("shell_232")
$.blockInput := Func("shell_231")
$.ceil := Func("shell_230")
$.chunk := Func("shell_229")
$.clearInterval := Func("shell_228")
$.clearTimeout := Func("shell_227")
$.click := Func("shell_226")
$.clone := Func("shell_225")
$.close := Func("shell_224")
$.compact := Func("shell_223")
$.concat := Func("shell_221")
$.confirm := Func("shell_220")
global __iDebounce__ := 0
global __tDebounce__ := {}
$.debounce := Func("shell_219")
$.defer := Func("shell_217")
$.delay := Func("shell_215")
$.drop := Func("shell_213")
$.each := Func("shell_211")
class EmitterShell {
  bus := []
  emit := Func("shell_210").Bind(this)
  off := Func("shell_206").Bind(this)
  on := Func("shell_203").Bind(this)
}
...
shell_204(__type__, __it__) {
  return __it__[1] != __type__
}
shell_205(__type__, __name__, __it__) {
...
shell_206(this, key) {
  ...
  if (__name__) {
    this.bus := $.filter.Call(this.bus, Func("shell_205").Bind(__type__, __name__))
  } else {
    this.bus := $.filter.Call(this.bus, Func("shell_204").Bind(__type__))
  }
}
shell_207(args, __it__) {
  __it__[3].Call(args*)
}
shell_208(__type__, __it__) {
  return __it__[1] == __type__
}
shell_209(__type__, __name__, __it__) {
  return __it__[1] == __type__ && __it__[2] == __name__
}
shell_210(this, key, args*) {
  __array__ := $.split.Call(key, ".")
  __type__ := __array__[1]
  __name__ := __array__[2]
  if !(__type__) {
  ...
  if (__name__) {
    __list__ := $.filter.Call(this.bus, Func("shell_209").Bind(__type__, __name__))
  } else {
    __list__ := $.filter.Call(this.bus, Func("shell_208").Bind(__type__))
  }
  $.map.Call(__list__, Func("shell_207").Bind(args))
}
shell_211(list, callback) {
  for __i__, __item__ in list {
    __i__ := __i__ - 1
    callback.Call(__item__, __i__)
  }
...
shell_213(list, n := 1) {
  return $.filter.Call(list, Func("shell_212").Bind(n))
}
shell_214(callback, args) {
  callback.Call(args*)
}
shell_215(callback, time, args*) {
  __timer__ := $.setTimeout.Call(Func("shell_214").Bind(callback, args), time)
  return __timer__
}
shell_216(callback, args) {
  callback.Call(args*)
}
shell_217(callback, args*) {
  return $.delay.Call(Func("shell_216").Bind(callback, args), 1)
}
shell_218(callback, time, index) {
...
shell_219(time, callback) {
  __iDebounce__++
  return Func("shell_218").Bind(callback, time, __iDebounce__)
}
shell_220(text, callback) {
...
shell_223(list) {
  return $.filter.Call(list, Func("shell_222"))
}
shell_224(target) {
  if !(target) {
...
shell_235(message*) {
  __msg__ := $.join.Call(($.map.Call(message, Func("shell_234"))), ", ")
  MsgBox, % __msg__
}
shell_236(args*) {
  __result__ := 0

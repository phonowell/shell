...
global __ci_shell__ := Func("shell_156")
global $ := {}
$.abs := Func("shell_155")
$.add := Func("shell_154")
$.alert := Func("shell_153")
$.beep := Func("shell_151")
$.bind := Func("shell_150")
$.blockInput := Func("shell_149")
$.ceil := Func("shell_148")
$.chunk := Func("shell_147")
$.clearInterval := Func("shell_146")
$.clearTimeout := Func("shell_145")
$.click := Func("shell_144")
$.clone := Func("shell_143")
$.compact := Func("shell_142")
$.concat := Func("shell_140")
global __iDebounce__ := 0
global __tDebounce__ := {}
$.debounce := Func("shell_139")
$.defer := Func("shell_137")
$.delay := Func("shell_135")
$.drop := Func("shell_133")
$.each := Func("shell_131")
class EmitterShellX {
  bus := []
  emit := Func("shell_130").Bind(this)
  off := Func("shell_129").Bind(this)
  on := Func("shell_128").Bind(this)
}
$.emitter := Func("shell_127")
global __eq__ := Func("shell_126")
$.eq := __eq__
$.exit := Func("shell_125")
class FileShellX {
  name := ""
  ...
  append := Func("shell_124").Bind(this)
  exist := Func("shell_123").Bind(this)
  delete := Func("shell_122").Bind(this)
  load := Func("shell_121").Bind(this)
  save := Func("shell_120").Bind(this)
}
$.file := Func("shell_119")
$.filter := Func("shell_118")
$.findColor := Func("shell_117")
$.findImage := Func("shell_116")
$.first := Func("shell_115")
$.head := $.first
$.floor := Func("shell_114")
$.formatHotkey := Func("shell_113")
global __formatKeyFormatHotkey__ := Func("shell_112")
global __pickPrefixFormatHotkey__ := Func("shell_111")
$.getColor := Func("shell_110")
$.getPosition := Func("shell_109")
$.getState := Func("shell_108")
$.gt := Func("shell_107")
$.gte := Func("shell_106")
$.i := Func("shell_105")
$.includes := Func("shell_104")
$.info := Func("shell_103")
$.isArray := Func("shell_102")
$.isFunction := Func("shell_101")
$.isNumber := Func("shell_100")
$.isObject := Func("shell_99")
$.isString := Func("shell_98")
$.join := Func("shell_97")
class KeyBindingShellX {
  mapBound := {}
  mapCallback := {}
  add := Func("shell_96").Bind(this)
  fire := Func("shell_95").Bind(this)
  init := Func("shell_94").Bind(this)
  off := Func("shell_92").Bind(this)
  on := Func("shell_91").Bind(this)
  remove := Func("shell_90").Bind(this)
}
...
$.play := Func("shell_74")
$.press := Func("shell_73")
global __formatInputPress__ := Func("shell_72")
global __validateInputPress__ := Func("shell_71")
$.push := Func("shell_70")
$.random := Func("shell_69")
$.range := Func("shell_68")
$.reload := Func("shell_67")
$.replace := Func("shell_66")
$.reverse := Func("shell_65")
$.round := Func("shell_64")
$.setFixed := Func("shell_63")
$.setInterval := Func("shell_62")
$.setTimeout := Func("shell_61")
$.shuffle := Func("shell_60").Bind(__split__)
$.sleep := Func("shell_59")
$.slice := Func("shell_58")
global __pickArgumentSlice__ := Func("shell_57")
$.sort := Func("shell_56")
$.split := Func("shell_55")
$.sum := Func("shell_54")
$.suspend := Func("shell_53")
$.tail := Func("shell_52")
$.take := Func("shell_51")
global __iThrottle__ := 0
global __tThrottle__ := {}
global __tsThrottle__ := {}
$.throttle := Func("shell_50")
$.toLowerCase := Func("shell_47")
$.toLower := $.toLowerCase
$.toString := Func("shell_46")
$.toUpperCase := Func("shell_45")
$.toUpper := $.toUpperCase
$.trim := Func("shell_44")
$.trimEnd := Func("shell_43")
$.trimStart := Func("shell_42")
$.type := Func("shell_41")
$.uniq := Func("shell_40")
$.values := Func("shell_39")
$.VERSION := "0.0.10"
(Func("shell_38")).Call()
(Func("shell_37")).Call()
(Func("shell_36")).Call()
...
shell_11() {
  list := [1, 2, 3]
  listA := $.filter.Call(list, Func("shell_10"))
...
shell_18() {
  ...
  $.each.Call(list, Func("shell_17").Bind(listA, length))
...
shell_31() {
  ...
  string := $.join.Call(($.map.Call(listA, Func("shell_30"))), ",")
...
shell_42(input, omitting := " `t") {
  return LTrim(input, omitting)
}
shell_43(input, omitting := " `t") {
  return RTrim(input, omitting)
}
shell_44(input, omitting := " `t") {
...
shell_48(callback, index) {
  callback.Call()
  __tsThrottle__[__ci_shell__.Call(index)] := $.now.Call()
}
shell_49(callback, time, index) {
  $.clearTimeout.Call(__tThrottle__[__ci_shell__.Call(index)])
  __tThrottle__[__ci_shell__.Call(index)] := $.setTimeout.Call(Func("shell_48").Bind(callback, index), __tsThrottle__[__ci_shell__.Call(index)] - $.now.Call() + time)
}
shell_50(time, callback) {
  __iThrottle__++
  __tsThrottle__[__ci_shell__.Call(__iThrottle__)] := 0
  return Func("shell_49").Bind(callback, time, __iThrottle__)
}
shell_51(list, count := 1) {
...
shell_66(input, searchment, replacement, limit := -1) {
...
shell_77(callback) {
  ...
  return Func("shell_76").Bind(__i__, callback)
}
shell_78(list, n := 0) {
  if (n >= 0) {
...
shell_81(point := "", speed := 0) {
  MouseMove, point[1], point[2], speed
}
shell_82(dividend, divisor) {
...
shell_85(value, other) {
  return value <= other
}
shell_86(value, other) {
  return value < other
}
shell_87(input) {
  __type__ := $.type.Call(input)
...
shell_91(this, key, callback) {
  key := $.formatHotkey.Call($.replace.Call(key, ":down", ""))
  Hotkey, % key, % callback, On
  return $
}
shell_92(this, key, callback) {
  key := $.formatHotkey.Call($.replace.Call(key, ":down", ""))
...
shell_94(this, key) {
  ...
  this.mapCallback[__ci_shell__.Call(key)] := []
  __fn__ := Func("shell_93").Bind(key, this)
...
shell_106(value, other) {
  return value >= other
}
shell_107(value, other) {
  return value > other
}
shell_108(key) {
...
shell_117(color, start := "", end := "", variation := 0) {
  if !(start) {
    start := [0, 0]
  }
  if !(end) {
...
shell_133(list, n := 1) {
  return $.filter.Call(list, Func("shell_132").Bind(n))
}
shell_134(callback, args) {
  callback.Call(args*)
}
shell_135(callback, time, args*) {
  __timer__ := $.setTimeout.Call(Func("shell_134").Bind(callback, args), time)
  return __timer__
}
shell_136(callback, args) {
  callback.Call(args*)
}
shell_137(callback, args*) {
  return $.delay.Call(Func("shell_136").Bind(callback, args), 1)
}
...
shell_139(time, callback) {
  __iDebounce__++
  return Func("shell_138").Bind(callback, time, __iDebounce__)
}
shell_140(list, args*) {
...
shell_142(list) {
  return $.filter.Call(list, Func("shell_141"))
}
shell_143(input) {
  __type__ := $.type.Call(input)
  if (__type__ == "array") {
...
shell_153(message*) {
  __msg__ := $.join.Call(($.map.Call(message, Func("shell_152"))), ", ")
  MsgBox, % __msg__
}
shell_154(args*) {
  __result__ := 0

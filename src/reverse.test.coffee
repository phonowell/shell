...
unless reversed.Length() == 4 then throw new Error "REV001: Expected length 4, got #{reversed.length}"
unless reversed[0] == 4 and
    reversed[1] == 3 and
    reversed[2] == 2 and
    reversed[3] == 1
  throw new Error "REV002: Expected [4,3,2,1], got [#{reversed}]"

# Test 2: Original array should not be modified (immutable operation)
unless list[0] == 1 and list[1] == 2 and list[2] == 3 and list[3] == 4
  throw new Error "REV003: Original array was modified, got [#{list}]"
...
unless singleReversed.Length() == 1
  throw new Error "REV004: Expected length 1, got #{singleReversed.length}"
unless singleReversed[0] == 42 then throw new Error "REV005: Expected [42], got [#{singleReversed}]"
...
unless emptyReversed.Length() == 0
  throw new Error "REV006: Expected empty array, got length #{emptyReversed.length}"

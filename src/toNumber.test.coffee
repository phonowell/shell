...
unless result == 42 then throw new Error "T001: Expected 42, got #{result}"

# Test 2: Convert decimal string to number
decimalString = '3.14'
decimalResult = $toNumber decimalString
unless $math.abs(decimalResult - 3.14) < 0.0001 then throw new Error "T002: Expected 3.14, got #{decimalResult}"
...
unless trueResult == 1 then throw new Error "T005: Expected 1 for true, got #{trueResult}"

falseResult = $toNumber false
unless falseResult == 0 then throw new Error "T006: Expected 0 for false, got #{falseResult}"

# Test 6: Edge case - empty string
emptyResult = $toNumber ''
unless emptyResult == 0 then throw new Error "T007: Expected 0 for empty string, got #{emptyResult}"

# Test 7: Edge case - invalid string
# invalidResult = $toNumber 'abc'
# unless isNaN(invalidResult) then throw new Error "T008: Expected NaN for invalid string, got #{invalidResult}"

# Test 8: Edge case - null and undefined
nullResult = $toNumber null
unless nullResult == 0 then throw new Error "T009: Expected 0 for null, got #{nullResult}"

# undefinedResult = $toNumber undefined
# unless isNaN(undefinedResult) then throw new Error "T010: Expected NaN for undefined, got #{undefinedResult}"

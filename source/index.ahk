...
global __ci_shell__ := Func("shell_143")
global $ := {}
$.abs := Func("shell_142")
$.activate := Func("shell_141")
$.add := Func("shell_140")
$.alert := Func("shell_139")
$.beep := Func("shell_137")
$.bind := Func("shell_136")
$.blockInput := Func("shell_135")
$.ceil := Func("shell_134")
$.chunk := Func("shell_133")
$.clearInterval := Func("shell_132")
$.clearTimeout := Func("shell_131")
$.click := Func("shell_130")
$.clone := Func("shell_129")
$.close := Func("shell_128")
$.compact := Func("shell_127")
$.concat := Func("shell_125")
$.confirm := Func("shell_124")
global __iDebounce__ := 0
global __tDebounce__ := {}
$.debounce := Func("shell_123")
$.defer := Func("shell_121")
$.delay := Func("shell_119")
$.drop := Func("shell_117")
$.each := Func("shell_115")
class EmitterShell {
  bus := []
  emit := Func("shell_114").Bind(this)
  off := Func("shell_110").Bind(this)
...
shell_108(__type__, __it__) {
  return __it__[1] != __type__
}
shell_109(__type__, __name__, __it__) {
...
shell_110(this, key) {
  ...
  if (__name__) {
    this.bus := $.filter.Call(this.bus, Func("shell_109").Bind(__type__, __name__))
  } else {
    this.bus := $.filter.Call(this.bus, Func("shell_108").Bind(__type__))
  }
}
shell_111(args, __it__) {
  __it__[3].Call(args*)
}
shell_112(__type__, __it__) {
  return __it__[1] == __type__
}
shell_113(__type__, __name__, __it__) {
  return __it__[1] == __type__ && __it__[2] == __name__
}
shell_114(this, key, args*) {
  __array__ := $.split.Call(key, ".")
  __type__ := __array__[1]
  __name__ := __array__[2]
  if !(__type__) {
  ...
  if (__name__) {
    __list__ := $.filter.Call(this.bus, Func("shell_113").Bind(__type__, __name__))
  } else {
    __list__ := $.filter.Call(this.bus, Func("shell_112").Bind(__type__))
  }
  $.map.Call(__list__, Func("shell_111").Bind(args))
}
shell_115(list, callback) {
  for __i__, __item__ in list {
    __i__ := __i__ - 1
    callback.Call(__item__, __i__)
  }
...
shell_117(list, n := 1) {
  return $.filter.Call(list, Func("shell_116").Bind(n))
}
shell_118(callback, args) {
  callback.Call(args*)
}
shell_119(callback, time, args*) {
  __timer__ := $.setTimeout.Call(Func("shell_118").Bind(callback, args), time)
  return __timer__
}
shell_120(callback, args) {
  callback.Call(args*)
}
shell_121(callback, args*) {
  return $.delay.Call(Func("shell_120").Bind(callback, args), 1)
}
shell_122(callback, time, index) {
...
shell_123(time, callback) {
  __iDebounce__++
  return Func("shell_122").Bind(callback, time, __iDebounce__)
}
shell_124(text, callback) {
...
shell_127(list) {
  return $.filter.Call(list, Func("shell_126"))
}
shell_128(target) {
  if !(target) {
...
shell_139(message*) {
  __msg__ := $.join.Call(($.map.Call(message, Func("shell_138"))), ", ")
  MsgBox, % __msg__
}
shell_140(args*) {
  __result__ := 0
